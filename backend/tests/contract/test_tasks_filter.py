...
def mixed_status_tasks(db_session):
    ...
    tasks = [
        Task(title=f"Pending Task {i}", description=f"Desc {i}",
             due_date=date(2026, 4, 15 + i), priority="Medium", status="Pending")
        for i in range(3)
    ] + [
        Task(title=f"In Progress Task {i}", description=f"Desc {i}",
             due_date=date(2026, 4, 20 + i), priority="High", status="In Progress")
        for i in range(2)
    ] + [
        Task(title=f"Completed Task {i}", description=f"Desc {i}",
             due_date=date(2026, 4, 25 + i), priority="Low", status="Completed")
        for i in range(4)
    ]
...
def test_filter_by_pending_status(client, db_session, mixed_status_tasks):
    """
...
def test_filter_by_in_progress_status(client, db_session, mixed_status_tasks):
    """
...
def test_filter_by_completed_status(client, db_session, mixed_status_tasks):
    """
...
def test_filter_invalid_status(client, db_session):
    """
    Test GET /api/tasks?status=InvalidStatus returns 422 Unprocessable Entity.
...
def test_filter_returns_empty_when_no_matches(client, db_session):
    """
